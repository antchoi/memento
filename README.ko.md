...
- human, Hermes, optional external executor로 작업을 넘기되 self-report를 그대로 믿고 싶지 않을 때
...
- Hermes profile, Codex, Claude Code, OpenCode, OpenHands-style API worker 또는 다른 peer executor를 위한 optional extension boundary
...
아래 prompt를 Hermes Agent, Claude Code, Codex, OpenCode 또는 다른 local coding agent에 붙여 넣으세요.
...
직접 읽고 싶다면 [Installation Guide](docs/guide/installation.md)를 참고하세요. 이 문서는 agent가 실행하기 쉬운 checklist 형식으로 작성되어 있습니다: virtualenv 생성, dependency 설치, `doctor` 실행, smoke contract 실행, evidence 보고.
...
- [`docs/guide/installation.md`](docs/guide/installation.md) — agent-executable installation, dependency setup, Hermes plugin enablement, local agentmemory Docker integration
